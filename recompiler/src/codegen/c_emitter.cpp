...
GeneratedOutput generate_output(const ir::Program& program_in,
                                const uint8_t* rom_data,
                                size_t rom_size,
                                const GeneratorOptions& options) {
    GeneratedOutput output;

    // WebAssembly caps a single function body at ~7.65 MB. A few recompiled
    // routines are large connected control-flow regions that exceed that.
    // Work on a mutable copy and demote any oversized function: dropping it
    // from program.functions makes call sites fall through gb_dispatch ->
    // gb_interpret, which executes any address correctly (just interpreted).
    ir::Program program = program_in;
    {
        const size_t kMaxFunctionInstructions = 4096;
        std::vector<std::string> oversized;
        for (const auto& [name, func] : program.functions) {
            if (func.is_entry_point || func.is_interrupt_handler) continue;
            size_t icount = 0;
            for (auto bid : func.block_ids) {
                icount += program.blocks.at(bid).instructions.size();
            }
            if (icount > kMaxFunctionInstructions) oversized.push_back(name);
        }
        for (const auto& n : oversized) program.functions.erase(n);
        std::cerr << "[wasm-safe] demoted " << oversized.size()
                  << " oversized function(s) to the interpreter path\n";
    }
    ...
    // gb_dispatch is emitted (further below) as a set of chunk functions
    // plus a thin router, so that no single emitted function exceeds
    // WebAssembly's ~7.65 MB per-function limit. The address-sorted case
    // list is partitioned into chunks of kDispatchChunkSize cases each.
    const size_t kDispatchChunkSize = 1024;
    std::vector<uint16_t> dispatch_chunk_first;
    size_t dispatch_case_index = 0;
    size_t dispatch_chunk_count = 0;
    auto open_dispatch_chunk = [&](uint16_t first_addr) {
        dispatch_chunk_first.push_back(first_addr);
        // noinline: each chunk has a single call site (the router below), so
        // the optimizer would otherwise inline all chunks back into one
        // gb_dispatch — re-creating the oversized function we split apart.
        source_ss << "__attribute__((noinline)) static void gb_dispatch_chunk_"
                  << dispatch_chunk_count
                  << "(GBContext* ctx, uint16_t addr, uint8_t bank) {\n";
        source_ss << "        (void)bank;\n";
        source_ss << "        switch (addr) {\n";
    };
    auto close_dispatch_chunk = [&]() {
    ...
    // gb_dispatch and its chunk functions are emitted after the analysis below.
    ...
    for (auto& [addr, funcs] : addr_to_funcs) {
        if (dispatch_case_index == 0) {
            open_dispatch_chunk(addr);
        } else if (dispatch_case_index % kDispatchChunkSize == 0) {
            close_dispatch_chunk();
            open_dispatch_chunk(addr);
        }
        dispatch_case_index++;
    ...
    // Close the final chunk (open one first if the ROM had no functions).
    if (dispatch_chunk_first.empty()) {
        open_dispatch_chunk(0);
    }
    close_dispatch_chunk();

    // Emit gb_dispatch itself: the run loop + a router that forwards each
    // address to the chunk function owning its range.
    ...
    source_ss << "        }\n";
    for (size_t i = 0; i < dispatch_chunk_count; i++) {
        if (dispatch_chunk_count == 1) {
            source_ss << "        gb_dispatch_chunk_0(ctx, addr, bank);\n";
        } else if (i == 0) {
            source_ss << "        if (addr < 0x" << std::hex << std::setfill('0') << std::setw(4)
                      << dispatch_chunk_first[1] << std::dec
                      << ") gb_dispatch_chunk_0(ctx, addr, bank);\n";
        } else if (i + 1 == dispatch_chunk_count) {
            source_ss << "        else gb_dispatch_chunk_" << i << "(ctx, addr, bank);\n";
        } else {
            source_ss << "        else if (addr < 0x" << std::hex << std::setfill('0') << std::setw(4)
                      << dispatch_chunk_first[i + 1] << std::dec
                      << ") gb_dispatch_chunk_" << i << "(ctx, addr, bank);\n";
        }
    }
    source_ss << "    }\n";
    source_ss << "}\n\n";
